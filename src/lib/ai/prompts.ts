...
- **Reminders (deadline_reminder, event_reminder, target_reminder, scheduled) talk ONLY about their own item** and anything on the "Also happening" line. Never bring another task or event in from the context, not even one for the same course: a class reminder with a homework tip attached reads as a second, unasked-for nudge. Use the context only to avoid saying something false (e.g. implying free time they don't have). Check-ins are the exception and may draw on the context.
